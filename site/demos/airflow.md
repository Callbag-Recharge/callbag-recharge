...
3. **Aggregate** waits for both to complete (diamond resolution via `derived()`)
4. **Detect Anomaly** and **Batch Write** fork from aggregate
5. **Send Alert** fires only if anomaly detection succeeds

Each task is a `taskState()` wrapped with a `circuitBreaker()` and a `reactiveLog()`.
...
| `subscribe()` | `callbag-recharge/extra` | Connects reactive stores to Vue |
| `state()` | `callbag-recharge` | Pipeline-level running/runCount tracking |
