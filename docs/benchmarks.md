...
| Library | ops/sec | time (100K ops) |
|---|---|---|
| **callbag-recharge** | **43M** | 2.3ms |
| Preact Signals | 35M | 2.9ms |

Both perform an `Object.is` check and update. Recharge's output slot model dispatches DIRTY to a null output — effectively free. Custom `equals` functions are supported for structural equality.
...
With v4 STANDALONE mode, derived nodes eagerly connect at construction and stay connected. On dep change, DIRTY propagates via the output slot, followed by DATA with the recomputed value. Recharge is ~1.3x faster than Preact. The v3 advantage (4x) was larger because derived was lazy; v4 trades some throughput for always-current `get()` semantics.

### Computed/derived read (unchanged dependencies)

| Library | ops/sec | time (100K ops) |
|---|---|---|
| **callbag-recharge** | **160M** | 0.6ms |
| Preact Signals | 87M | 1.1ms |

STANDALONE derived nodes cache their last computed value. `get()` returns the cache directly — no recompute, no flag check. ~1.8x faster than Preact's cached flag check. Providing `equals` on a derived store enables push-phase memoization that skips downstream updates when the output hasn't changed.

### Diamond (A→B, A→C, B+C→D) write + read

| Library | ops/sec | time (100K ops) |
|---|---|---|
| Preact Signals | **9.7M** | 10.4ms |
| callbag-recharge | 7.5M | 13.4ms |

Both produce correct, glitch-free results. In v4, derived nodes are STANDALONE — each intermediate node maintains an active connection and output slot even without external subscribers. This adds overhead vs v3's lazy model but ensures `get()` always returns a current value. Preact's simpler push-invalidate model wins here by ~1.3x. Adding `equals` to intermediate derived stores can reduce unnecessary downstream propagation.

### Effect re-run

| Library | ops/sec | time (100K ops) |
|---|---|---|
| **callbag-recharge** | **27M** | 3.7ms |
| Preact Signals | 13M | 7.6ms |

Effects use static deps (connect once on creation, no reconnection per re-run). Integer bitmask dirty tracking and the pure closure implementation make recharge ~2x faster than Preact. Use `batch()` to coalesce multiple state changes into a single effect run.
...
| Library | ops/sec | time (100K ops) |
|---|---|---|
| **callbag-recharge** | **24M** | 4.2ms |
| Preact computed | 12M | 8.5ms |

`operator()` is the general-purpose transform primitive — receives all signal types from upstream and decides what to forward. Compared against Preact `computed` with one subscriber. The handler-based dispatch is ~2x faster than Preact.

### Pipe (3 operators) push through

| Library | ops/sec | time (100K ops) |
|---|---|---|
| raw Callbag | **93M** | 1.1ms |
| callbag-recharge `pipe` | 20M | 5.0ms |
...
|---|---|---|
| Inspector OFF | **6.4M** | 1.6ms |
| Inspector ON (default) | 1.1M | 9.0ms |

Disabling the Inspector skips `WeakRef` creation and `WeakMap` registration. Set `Inspector.enabled = false` in production. **~5.8x faster** store creation with Inspector disabled.

### batch() — 10 set() calls with effect

| Variant | ops/sec | time (10K ops) |
|---|---|---|
| Unbatched | **1.6M** | 6.1ms |
| Batched | 1.4M | 7.3ms |
...
With v4's optimized operator internals, `pipe` and `pipeRaw` perform roughly the same. `pipeRaw` fuses all transforms into a single `operator` store, eliminating intermediate nodes — the benefit is reduced store count and memory, not throughput. Use `pipeRaw` when you need the `SKIP` sentinel for filter semantics or want to minimize store allocations.

### equals on diamond intermediates

| Variant | ops/sec | time (100K ops) |
|---|---|---|
| Without equals | 8.0M | 12.5ms |
| With equals | 7.5M | 13.3ms |
...
- **State read** — 1.5x faster than Preact (180M vs 123M ops/sec)
- **State write** — 1.2x faster than Preact (43M vs 35M ops/sec)
- **Computed reads (unchanged deps)** — 1.8x faster than Preact (160M vs 87M ops/sec) — STANDALONE cache returns directly
- **Computed reads (after dep change)** — 1.3x faster than Preact (19M vs 15M ops/sec)
- **Effect re-run** — 2x faster than Preact (27M vs 13M ops/sec) thanks to bitmask dirty tracking and pure closure
- **Producer emit** — 1.7x faster than Preact (27M vs 16M ops/sec) thanks to lightweight output slot dispatch
- **Operator transform** — 2x faster than Preact (24M vs 12M ops/sec) using handler-based dispatch
...
- **Store creation** — 6.4M ops/sec with Inspector disabled (~5.8x faster than enabled)
...
- **Pipe throughput** — ~4.7x faster than recharge `pipe` because operators are just nested function calls with no store overhead
...
For typical application development, all three libraries are more than fast enough. callbag-recharge's value proposition is **inspectability + unified push/pull + correctness** at a performance level that beats Preact Signals in every benchmark except diamond patterns and memory per store.
