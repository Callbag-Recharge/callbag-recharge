...
import { signal, computed, effect as signalEffect, batch } from '@preact/signals-core';
import {
  state, derived, effect as rechargeEffect, pipe as rPipe, map as rMap,
  filter as rFilter, subscribe as rSubscribe, stream,
} from './src/index';
...
import { fromIter, interval } from 'callbag-basics';
...
function printGroup(title: string, results: Array<{ name: string; elapsed: number; opsPerSec: number }>) {
  console.log(`\n## ${title}`);
...
// ============================================================
{
  ...
  printGroup('State read', [
    bench('Preact signal()', () => { pSignal.value; }),
    bench('Recharge state.get()', () => { rState.get(); }),
  ]);
}
...
// ============================================================
{
  ...
  printGroup('State write (no subscribers)', [
    bench('Preact signal.value =', () => { pSignal.value = Math.random(); }),
    bench('Recharge state.set()', () => { rState.set(Math.random()); }),
  ]);
}
...
// ============================================================
{
  ...
  printGroup('Computed read after dep change', [
    bench('Preact computed', () => { pA.value = pi++; pSum.value; }),
    bench('Recharge derived', () => { rA.set(ri++); rSum.get(); }),
  ]);
}
...
// ============================================================
{
  ...
  printGroup('Computed read (unchanged deps)', [
    bench('Preact computed (cached)', () => { pSum.value; }),
    bench('Recharge derived (recomputes)', () => { rSum.get(); }),
  ]);
}
...
// ============================================================
{
  ...
  printGroup('Diamond (A→B,C→D) write + read', [
    bench('Preact signals', () => { pA.value = pi++; pD.value; }),
    bench('Recharge stores', () => { rA.set(ri++); rD.get(); }),
  ]);
}
...
// ============================================================
{
  ...
  let pRuns = 0;
  signalEffect(() => { pTrigger.value; pRuns++; });
  let pi = 0;

  const rTrigger = state(0);
  let rRuns = 0;
  rechargeEffect(() => { rTrigger.get(); rRuns++; });
  let ri = 0;

  printGroup('Effect re-run', [
    bench('Preact effect', () => { pTrigger.value = pi++; }),
    bench('Recharge effect', () => { rTrigger.set(ri++); }),
  ]);
}
...
// ============================================================
{
  ...
  cbPipe(
    ...
    cbMap((n: number) => n + 1),
    cbSubscribe((v: number) => { cbResult = v; }),
  );

  let ci = 1;
  printGroup('Pipe (3 operators) push through', [
    bench('Recharge pipe', () => { rSrc.set(ri++); rPiped.get(); }),
    bench('Callbag pipe', () => { cbEmitter!(ci++); }),
  ]);
}
...
{
  const pSrc = signal(0);
  for (let i = 0; i < 10; i++) signalEffect(() => { pSrc.value; });
  let pi = 0;
  ...
  printGroup('Fan-out (10 subscribers)', [
    bench('Preact 10 effects', () => { pSrc.value = pi++; }),
    bench('Recharge 10 subscribe', () => { rSrc.set(ri++); }),
  ]);
}
...

{
  ...
  console.log(`| Preact signal | ~${perStore.toFixed(0)} | ${((after.heapUsed - before.heapUsed) / 1024).toFixed(0)} KB |`);
...

{
  ...
  const after = process.memoryUsage();
  const perStore = (after.heapUsed - before.heapUsed) / 10_000;
  console.log(`| Recharge state | ~${perStore.toFixed(0)} | ${((after.heapUsed - before.heapUsed) / 1024).toFixed(0)} KB |`);
  stores.length = 0;
}
