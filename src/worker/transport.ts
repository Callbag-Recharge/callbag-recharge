...
// ---------------------------------------------------------------------------
// Abstracts Worker, SharedWorker, ServiceWorker, and BroadcastChannel behind
// a uniform post/listen/terminate interface. Auto-detection via instanceof.
// ---------------------------------------------------------------------------
 ...
 * - `BroadcastChannel` — postMessage/onmessage
 * - `MessagePort` — direct postMessage/onmessage (worker-side SharedWorker port)
 *
 * @param target - The worker, port, or channel to wrap.
 * @returns A WorkerTransport with normalized post/listen/terminate.
 */
export function createTransport(target: unknown): WorkerTransport {
		...
		return {
			post(data, transfer?) {
				if (transfer && transfer.length > 0) {
					console.warn(
						"[callbag-recharge] WorkerTransport: BroadcastChannel does not support Transferable objects. The transfer argument is ignored and objects will be cloned instead.",
					);
				}
	...
	// ServiceWorker (from navigator.serviceWorker.controller or ServiceWorkerRegistration.active)
	if (typeof ServiceWorker !== "undefined" && target instanceof ServiceWorker) {
		return {
			...
			listen(handler) {
				// ServiceWorker messages arrive on navigator.serviceWorker (the container).
				// Filter by source to avoid cross-contamination when multiple SW transports exist.
			...
			// ServiceWorkers can't be terminated from the page
		};
	}
