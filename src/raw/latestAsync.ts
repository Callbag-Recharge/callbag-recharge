...
// is delivered. Earlier in-flight results are silently discarded. Each call
// receives an AbortSignal that fires when a newer call supersedes it or
// cancel() is called — use it with firstValueFrom({ signal }) to clean up
...
//   latest.call('world', result => doSomething(result)); // 'hello' aborted + discarded
//   latest.cancel(); // aborts + invalidates any remaining in-flight call
// ---------------------------------------------------------------------------
 ...
 *
 * @param fn - The async function to wrap. Receives `(input, signal)`.
 *
...
export function latestAsync<TInput, TResult>(
	...
	return {
		call(input, onResult, onError) {
			// Abort the previous in-flight call
			controller?.abort();
			controller = new AbortController();
			const gen = ++generation;
			const signal = controller.signal;
			fn(input, signal).then(
				(result) => {
